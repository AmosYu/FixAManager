...
package fam.ctl.fixamanager.zxing.zxing.view;
...
public final class ViewfinderView extends View {
    ...
    private long ANIMATION_DELAY = 10L;
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (frame == null) {
            ...
            int width = CameraManager.MAX_FRAME_WIDTH;
            int height = CameraManager.MAX_FRAME_HEIGHT;
...
//            paint.setColor(frameColor);
    ...
    private void drawText(Canvas canvas, Rect frame) {
        if (cameraPermission == PackageManager.PERMISSION_GRANTED) {
            ...
            int startX = (getMeasuredWidth() / 2) - (getStringWidth(hint, paint) / 2);
            canvas.drawText(hint, startX, frame.bottom + 35 + 20, paint);
        } else {
            paint.setColor(errorHintColor);
            paint.setAntiAlias(true);
            paint.setTextSize(28);
            int startX = (getMeasuredWidth() / 2) - (getStringWidth(errorHint, paint) / 2);
            canvas.drawText(errorHint, startX, frame.bottom + 35 + 20, paint);
        }
    }
    ...
    private void drawScanner(Canvas canvas, Rect frame) {
        if (showPossiblePoint) {
            ...
            translateY += 5f;
            if (translateY >= (CameraManager.MAX_FRAME_HEIGHT - 5)) {
                translateY = 5f;
            }
        }
    }
